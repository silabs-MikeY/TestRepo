...
void sl_button_on_change(const sl_button_t *handle)
{
  ...
  if (SL_SIMPLE_BUTTON_INSTANCE(1) == handle) {
    uint32_t bootloader_choice = BTL_SMP_APP_ID_2;
    app_log_info("Setting Switch Record to: %d" APP_LOG_NL, bootloader_choice);
    bool result = btl_smp_switch_request_next_boot_app(bootloader_choice);
    app_log_info("Switch Record set result: %d" APP_LOG_NL, result);
    if (result) {
      app_log_info("Resetting Device" APP_LOG_NL);
      NVIC_SystemReset();
    }
  }
}
